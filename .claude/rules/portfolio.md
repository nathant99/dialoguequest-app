...
**⚠ THE PER-APP "Ceiling" COLUMN BELOW IS KNOWN-UNDERESTIMATED (~5×, founder-measured 2026-08-09) — do NOT trust it for a gen-budget decision.** The numbers are hand-guessed **$/image** figures that omit the billed THINKING tokens (Nano Banana Pro is a Gemini-3-Pro thinking model, +$0.05–0.10/call), the bundled TTS/audio step, input+reference-image tokens, retries, and resolution. Estimate cost the CORRECT way — from `usageMetadata` tokens × current per-modality rates, reconciled against Cloud Billing (real spend: Jun $433 / Jul $894 / Aug ~$1.3K-pace, Tier-3) — per `spark-anvil-website.md` § R-GEMINI-COST-ESTIMATION + `Docs/RESEARCH_GEMINI_COST_ESTIMATION_2026-08-09.md`. These ceilings will be RE-DERIVED from measured `usageMetadata` (tracked build); until then treat them as ~5× low.

**Covered asset classes** (non-exhaustive; hub owns ALL):

| Class | Pipeline | Vendor | Ceiling (per-app) ⚠~5× LOW |
|---|---|---|---|
...
Cost discipline: estimate every wave's cost the CORRECT way (token-based from `usageMetadata`, reconciled against Cloud Billing — `spark-anvil-website.md` § R-GEMINI-COST-ESTIMATION), NOT from the ~5×-low ceilings above; a gen script should emit its own per-run cost line. New asset classes added to this table when first vendor pipeline is exercised; cost estimate set per founder approval BEFORE first gen run.
...
- **⚠ To VERIFY a coverage-guard's state, run it READ-ONLY — NOT `--write-registry` (2026-08-08).** `--write-registry` REGENERATES the committed registry, and it preserves ONLY the machine-parseable content (the data rows + `# XXX-EXCLUDED:`/`# XXX-DEFERRED:` lines) — it silently DROPS any custom PROSE header (e.g. a `# VF-REVIEW-BASIS`/`# …-BASIS` block documenting the verdict grounding). So a session that runs `--write-registry` merely to confirm a count loses the header. Run the guard with no flag (or `--ci-mode`) to verify; use `--write-registry` only when you intend a regen, and RE-INSERT any custom prose header afterward (bit the q-4c8a50 visual-first review flip — the `# VF-REVIEW-BASIS` block was dropped + restored). The tell: a `--write-registry` diff with MORE deletions than data-row changes = a dropped comment block.
...
## Spawning a shared LIBRARY repo (Swift package / Rust crate — ForgeKit-pattern) uses the LIBRARY scaffold, NOT the app-spawn scaffold; hub scaffolds + delivers data/tooling, the repo's own CC session writes the Swift/Rust SOURCE (R-LIBRARY-REPO-SPAWN; 2026-08-02)

**When the founder greenlights standing up a shared portfolio LIBRARY — a Swift package or Rust crate that mirrors the ForgeKit pattern (e.g. the CubeKit tracks `cubekit`/`cubekit-geo`/`cubekit-rs`, ADR-089), NOT an educational app — HUB creates the repo (`gh repo create`, **private**) + scaffolds it to the LIBRARY standard, delivers any DATA/TOOLING substrate, and the repo's OWN CC session writes the Swift/Rust SOURCE (hub never writes source). The load-bearing distinction: a library gets the ForgeKit LIBRARY scaffold, NEVER the R-APP-SPAWN-PLAYBOOK app scaffold — the app-only pieces (16×25 kits · the DN-S cast/portraits/chapters/audio workflow · the app-only rules `distributed-narrative`/`trauma-informed`/`spark-anvil-website`/`cognitive-accessibility` · CLAUDE.md-template-v2 · `Resources/Questions` · the R-SPAWN-COMPLETENESS-DIMENSIONS content dims) are DELIBERATELY ABSENT (a library is not an educational app), and the library is NOT added to `REGISTRY_ACTIVE_PORTFOLIO_APPS.txt` (it's infrastructure, like ForgeKit/forgesync — CLAUDE.md § Infrastructure repos).** Codified per founder-direct 2026-08-02 (*"does the 3 cubekit repos have all the doc set and rules … like other portfolio repos? … i meant library spawn process"*). This is the LIBRARY sibling of § R-APP-SPAWN-PLAYBOOK.

**The LIBRARY scaffold standard (mirror ForgeKit; hub authors all of it — non-source):**
1. **`README.md` · `LICENSE` (MIT, open-source per the ecosystem-gap rationale) · `.gitignore`** · a library-specific **`CLAUDE.md`** (the library's identity + build/consume guidance — NOT the app template v2).
2. **`CHANGELOG.md`** (Keep a Changelog + SemVer) — the versioning spine (ForgeKit convention).
3. **A module/crate CATALOG** — `Docs/MODULE_CATALOG.md` (Swift) or `CRATE_LAYOUT.md` (Rust) — the layout + public API map.
4. **`Docs/APPROACH.md`** (+ any design docs) — the track's scope, trade-offs, build targets.
5. **`.claude/rules/` — the LANGUAGE-appropriate SUBSET** (this is the piece most easily forgotten; a library repo without `.claude/rules/` is an incomplete scaffold):
   - **Swift package** → the Swift-dev + workflow + cross-repo subset from ForgeKit's own `.claude/rules/`: `concurrency · spm-architecture · state-machines · swiftui · swiftdata · spritekit · swiftlint · testing · warnings · build-optimization · localization · foundationmodels · workflow · portfolio`. (Skip the app/instance-specific ones — DN/trauma/website/cognitive-accessibility, and ForgeKit's own `forgekit.md` module catalog.)
   - **Rust crate** → the Swift rules DON'T apply; ship `workflow · portfolio` (universal) + an **authored `rust-conventions.md`** (Rust 2021 · `cargo fmt`/`clippy -D warnings` · WASM via `wasm-pack` + iOS via UniFFI/`cargo-xcframework` · **never panic across the FFI boundary** · the 10k-random-solve invariant DoD · **share the geometry/logic DATA, never the renderer**, ADR-089 D2).
6. **The package manifest + SOURCE (`Package.swift`/`Cargo.toml` + `Sources/`/`src/` + tests) is the repo's OWN CC session's work — HUB NEVER WRITES IT** (CLAUDE.md § CRITICAL never-write-source). A `.mjs` data/build TOOL + generated DATA (e.g. `tools/export_geometry.mjs` + `geometry-data/*.json`) is hub-owned TOOLING/RESOURCE, not source (§ R-HUB-OWNS-APP-RESOURCES).
7. **Flip the repo PUBLIC (MIT) only when real + tested** — never publish an empty scaffold.

**The multi-track pattern (ADR-089 reference).** A library standup MAY fork into parallel evaluation tracks (CubeKit = Swift package · hybrid-Rust-geometry · full-Rust — "progress all 3 simultaneously, pick the winner"); each track is its own repo, scaffolded identically to the standard above (Swift subset for the Swift track; Rust subset for the Rust tracks), built by its own session, evaluated, then one wins + the consumer migrates its dependency.

**Adopting an EXTRACTED library (the consumer side — the parity proof).** When an app adopts a library that was extracted FROM it (ForgeKit, CubeKit), the migration is the app session's Swift work (hub files the adoption handoff), and three sequencing facts are load-bearing: **(a) LOCAL-PATH-FIRST for parity** — the DoD "verify behavior parity" (solves/scrambles/decode identical) does NOT need the library tagged, API-curated, or public, so adopt via `.package(path: "…")` NOW to prove parity, THEN switch to `.package(url:…, from:"<tag>")` once the library curates its API → tags a version → (optionally flips public MIT). "Adopt now" + "curate/tag/flip later" are compatible via this order — never block the parity proof on a release. **(b) SAME-NAMED-TARGET COLLISION** — an extracted library exposes targets with the SAME names as the app's internal ones (`CubeModels`/`CubeEngine`/…); the app CANNOT depend on the library while keeping its same-named internal targets, so the migration DELETES the internal targets as it switches to `import <Library>`, and **re-homes the app-specific types the library intentionally dropped (ADR-086 D4 scope) into a DIFFERENTLY-named app module** — never a second `CubeModels`. **(c) readiness = the app has its internal engine to migrate FROM + the dropped types already present + the specs; the *blockers* live on the LIBRARY side (tag · curated public surface · visibility)** — so "is the app ready to adopt?" is answered by ground-truthing BOTH repos, not by assuming the library is a consumable release (R-CANONICAL-DOC-GROUND-TRUTH / R-APP-IMPL-READINESS-CHECK applied to a dependency swap).

**Standing ARCHITECTURE-HEALTH audit (the R-HUB-REPO-HYGIENE / R-SITE-REPO-HYGIENE sibling for libraries; grounded in 2026 SPM best practices).** A shared library repo gets a periodic architecture-health pass — hub AUDITS + files a handoff (the reorg is Swift/Rust SOURCE → the repo's OWN session applies it, hub never moves source), checking, in priority order: **(a) module BOUNDARIES are clean** (real SPM modules, a clean dependency hierarchy — the PRIMARY lever per Point-Free; a folder is not a boundary) — usually already right for a ForgeKit-pattern lib; **(b) no GOD-FILES** (scan the largest source files — a 1,000-line file is the real anti-pattern, NOT a many-flat-file folder); **(c) FLAT-HEAVY modules** (all files dumped in a module's root, zero subfolders — a *readability* smell, not an architecture defect; subfolder by concern WITHIN the target — no `Package.swift`/API change, since only the first level under `Sources/` maps to a target); **(d) an API-BREAK guard** for a consumed/open-source library (`swift package diagnose-api-breaking-changes` / an API-diff in CI + CHANGELOG + SemVer version-core tags); **(e) module-count / catalog DOC-DRIFT** (the rule/CLAUDE.md counts vs the live `Sources/` tree — R-CANONICAL-DOC-GROUND-TRUTH; reconcile the hub-owned rule docs, verify against the repo's authoritative CHANGELOG). **Honest framing:** a well-architected framework's findings are usually readability polish, not defects — say so plainly (ForgeKit 2026-08-02 = 59 clean modules + no god-files → strong; only CubeKit's 81-flat-file `CubeModels` was a real cleanup). Reference audit: `Docs/AUDIT_LIBRARY_REPOS_ARCHITECTURE_2026-08-02.md`.

**When it applies:** standing up ANY shared library (Swift/Rust, ForgeKit-pattern); auditing a library repo's completeness ("does it have the docs/rules like other portfolio repos?" → check the 7-item standard, NOT the app-spawn set) OR its architecture health (the 5-point audit above). **Cross-refs:** § R-APP-SPAWN-PLAYBOOK (the APP sibling — different scaffold) · § R-HUB-REPO-HYGIENE + `spark-anvil-website.md` § R-SITE-REPO-HYGIENE (the repo-health siblings) · `.claude/rules/forgekit.md` (the pattern the library mirrors) · `Docs/ADR-089_CUBESENSEI_CUBEKIT_LIBRARY_AND_GENERIC_GEOMETRY.md` (the reference instance) · `Docs/AUDIT_LIBRARY_REPOS_ARCHITECTURE_2026-08-02.md` · CLAUDE.md § CRITICAL (never-write-source) + § Infrastructure repos (why it's not in the app registry) · § R-HUB-OWNS-APP-RESOURCES (data/tooling ownership) · `workflow.md` § R-CANONICAL-DOC-GROUND-TRUTH (the doc-drift discipline).
...
All portfolio apps share a common SPM framework at `../forgekit/` (**59 modules** — 46 client + 11 server + 2 shared, live 2026-08-02; pre-1.0, verify `forgekit/Docs/CHANGELOG.md`; sources soft-split into `Client/` + `Server/` + `Shared/`). Apps import only the modules they need. See `@.claude/rules/forgekit.md` for the full module catalog.
