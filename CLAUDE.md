...
- **`Bundle.url(forResource:"", withExtension:"webp")` returns the FIRST matching-extension file in the bundle, NOT nil** — empty resource name behavior is intentionally undefined by `Bundle.url(forResource:withExtension:)`; an empty string matches indiscriminately. Test surfaces guarding "unknown slug returns nil" must use a recognizably-typo'd token (`"not-a-cast-member"`), NOT empty-string. Codified 2026-06-23 (second mid-session) writing `CastPortraitImage` tests against the bundled cast portraits.
- **String extensions in default-MainActor SPM packages inherit MainActor unless explicitly `nonisolated`** — even private `extension String { func ... }` methods on a `nonisolated public struct`'s implementation surface inherit MainActor from the package's default-isolation rule (per `.claude/rules/concurrency.md` § "Extension methods inherit MainActor too"). Compiler error: "Call to main actor-isolated instance method 'X' in a synchronous nonisolated context." Fix: mark every extension method `nonisolated`. Codified 2026-06-23 (second mid-session) from `TraumaAxisAdvisoryService.swift`'s `containsCueToken(_:)` extension method.
- **SPM `.process()` resource paths are relative to the TARGET's source root, not the repo root** — when hub distributes assets to repo-root `Resources/Cast/` + `Resources/CustomArt/`, the SPM target consuming them needs the files under `Libraries/Sources/<Target>/Resources/<subdir>/` AND the `.process("Resources/<subdir>")` rule in `Libraries/Package.swift`. Copying hub-shipped assets into the target's Resources tree (not just leaving them at repo root) is what makes `Bundle.module.url(forResource:withExtension:)` succeed. Codified 2026-06-23 (second mid-session) closing `HANDOFF_FROM_HUB_CAST_PORTRAITS.md` + `HANDOFF_FROM_HUB_BOOK_COVERS.md`.
...
- **`ForgeMasteryEngine.NextProblemPicker.recommendations(state:)` is NON-DETERMINISTIC across calls, and a fresh state still surfaces a `.stretch`** — the extend/stretch selection iterates `MasteryGraph.frontierTopics(...)`, which returns a `Set` (unordered). On a tie (two unblocked pillars at the same attemptCount/score), two sequential calls can return DIFFERENT topics. Any surface that needs internal consistency (a "focus" line + a card list that must agree) MUST call `recommendations()` ONCE and derive everything from that single array — never call it twice expecting parity. Also: a brand-new install (empty state) is NOT empty — `.stretch` recommends the first unblocked, unattempted pillar as a starting suggestion, so `nextFocusTopic()` is non-nil from launch (gate kid-facing surfaces on a separate "has published" signal, not on `nextFocusTopic() != nil`). Reference impl: `DialogueCraftMasteryService.recommendationReadouts()` calls `recommendations()` once. Codified 2026-07-10 (twenty-first mid-session) deepening the `ForgeMasteryEngine` surface.
- **`ForgeMasteryEngine.TopicMasteryState.masteryScore` is time-decayed (FSRS retrievability term)** — two reads microseconds apart differ at ~1e-9, so NEVER assert bit-exact equality between a `masteryScore(for:)` call and a `masteryReadouts()` row (each calls `decoded()` at a slightly different `Date()`). Assert the meaningful invariant instead (`>= masteryThreshold` / `isMastered == true`). Codified 2026-07-10 (twenty-first mid-session) writing `DialogueCraftMasteryServiceTests` readout coverage.
- **`NextProblemPicker.recommendations(state:)` can return EMPTY after attempts even though a fresh install never does** — a brand-new install reliably surfaces a `.stretch` (the FSRS frontier always offers a starting suggestion), so `nextFocusTopic()` / `nextFocusNudge()` are non-nil from launch. But once the kid has published a few trees, the picker can legitimately return NO recommendation (nothing at the edge-of-competence band, nothing decayed enough to consolidate, nothing newly unblocked) — so `nextFocusNudge()` becomes `nil` in that state. Consequence for TESTS: assert the "names a pillar" invariant against a FRESH service (deterministic `.stretch`), NOT a post-publish state (non-deterministically empty). Consequence for SURFACES: any reader that must show a focus line should tolerate `nil` (the `WriteTabView` bubble slot simply doesn't fire; the Progress-tab caption hides). Codified 2026-07-11 (twenty-second mid-session) wiring the mastery-focus nudge into the live Patter bubble — a `focusNudgeLineNamesARecommendedPillar` test failed when it published first, then passed against a fresh service.
