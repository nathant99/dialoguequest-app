...
- [ ] Add 5 more triangle-specific question kits (kits 05-09) — kits 05 + 06 + 07 SHIPPED (`kit_05_triangle_voices.json` — alliance / jealousy / arbitration + voice differentiation, 2026-06-22; `kit_06_voice_crucible.json` — voice fingerprinting + register matching + stakes vs surface text, 2026-06-22; `kit_07_subtext_crucible.json` — multi-listener subtext + silence-as-subtext + two-layer body-vs-words, 2026-06-23; all registered via `QuestionKitLoader.phase2Kits` + `.allKits` + `.loadAllPhases()`). Kit 07 also surfaces the Phase 2 `MultiSpeakerSubtextAnalysis` AI shape to kids in playable form for the first time. Kits 08-09 stay queued until 07 surfaces gaps in multi-listener subtext craft.
