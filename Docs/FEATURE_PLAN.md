...
- [ ] Add 5 more triangle-specific question kits (kits 05-09) — kit 05 SHIPPED 2026-06-22 (`Resources/Questions/kit_05_triangle_voices.json` — 5 questions on alliance / jealousy / arbitration + voice differentiation across 3 characters; `QuestionKitLoader.phase2Kits` + `.allKits` + `.loadAllPhases()` register it). Kits 06-09 stay queued until kit 05 surfaces real triangle-craft gaps.
- [ ] Add 8 Phase-2 achievements — 4 SHIPPED 2026-06-22 (`triangle_published` / `triangle_three_voice_spread` / `triangle_alliance_pattern` / `triangle_arbitration_pattern` — all in `DialogueQuestGamification.phase2Achievements`). `AchievementService.Criteria` extends with `characterCount` + `trianglePattern: TrianglePattern` + `hasThreeVoiceSpread`; `satisfiesCriteria` gates each Phase 2 badge on the triangle path (characterCount ≥ 3 AND totalNodes ≥ 5). Phase 1 regression-guard test confirms a 2-character publish never awards a Phase 2 badge. Remaining 4 Phase-2 badges land alongside kits 06-09.
