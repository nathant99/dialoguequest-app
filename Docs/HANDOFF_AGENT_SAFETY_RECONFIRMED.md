# Agent Safety — Reaffirmed (2026-06-26)
...
| `Docs/HANDOFF_TO_USER_FORGEKIT_PIN_BUMP.md` | NEW 2026-06-26 — bump ForgeKit pin in `Libraries/Package.swift` from `from: "0.99.0"` to a 1.0.0-rc.x-aware constraint (e.g. `.exact("1.0.0-rc.3")` or `.upToNextMajor(from: "1.0.0-rc.3")` if you want patch-level updates). Unblocks `ForgeMasteryEngine` + `PolyaScaffold` integration. |
| `Docs/HANDOFF_TO_USER_LOCALIZABLE_XCSTRINGS.md` | NEW 2026-06-26 — File → New File → String Catalog → save as `Libraries/Sources/AppFeature/Resources/Localizable.xcstrings`. Pre-App-Store localization seam; unblocks `ForgeLocalization` adoption. |
